...
import {
  ...
  useRandomVisitor,
  Visitor,
} from "services/counter-booth/hooks";
...
const Randomizer2: NextPage = () => {
  ...
  const { data: visitorData, isLoading } = useRandomVisitor({
    boothIds: selBooths,
    webinarIds: selWebinars,
    winners: winners.map((w) => w.visitor?.id || 0),
  });

  const getManualData = () => {
    const list = manualCandidates
      .split("\n")
  ...
  const rewardOpts = useMemo(
    () =>
      settings?.doorprize?.rewards.map((reward) => ({
        value: `${reward.id}`,
        label: reward.name,
      })),
    [settings]
  );
  const reward = (idx: string | number) =>
    settings?.doorprize?.rewards[parseInt(`${idx}`)];
  ...
  const handleParamsChange = ({ selBooths, selWebinars }: SelProps) => {
    if (selBooths) {
      if (selBooths.length > 1) {
        ...
        if (
          ["one", "all"].includes(firstBooth) ||
          ["one", "all"].includes(lastBooth)
        ) {
          selBooths = [lastBooth];
        }
    ...
    if (selWebinars) {
      if (selWebinars.length > 1) {
        ...
        if (
          ["one", "all"].includes(firstWebinar) ||
          ["one", "all"].includes(lastWebinar)
        ) {
  ...
  const calcRandomizerN = (delta: number, f = 60) =>
    Math.round(
      (0 -
        1 -
        2 * f +
        Math.sqrt(Math.pow(1 + 2 * f, 2) + 8 * (delta + 780 + f * 40))) /
        2 -
        40
    );
  ...
  const startRandomizing = () => {
    ...
    searchWinner(
      0,
      calcRandomizerN(settings?.doorprize?.randomizer_time || 5000)
    );
  };
  ...
  const censorNumber = (mobile: string | undefined) =>
    mobile?.replace(/(\d{4})\d{0,6}(\d{4})/, "$1****$2");
            ...
            <Center>
              <Paper>
                ...
                <Group direction="column" align="center" spacing={4} mt="xs">
                  ...
                  <Button
                    loading={isRandomizing || isLoading}
              ...
              <Paper>
                ...
                <Group direction="column" align="center" spacing={4} mt="xs">
                  {rewardSrc(selReward) && (
                    <Image
                      mt="lg"
                      src={rewardSrc(selReward)}
                      alt={rewardName(selReward)}
                      height={200}
                    />
                  )}
                </Group>
                  ...
                  <Group direction="column" align="center" spacing={4} mt="xs">
                    {tempWinner && (
                      <div className={classes.winnerContainer}>
                        <Text className={classes.tempWinner}>
                          {tempWinner?.name}
                        </Text>
                      </div>
                    )}
                    {wlWinner && (
                      <div className={classes.winnerContainer}>
                        ...
                        <Text className={classes.value} align="center">
                          {wlWinner.visitor?.name}
                        </Text>
                        <Text align="center">
                          {censored
                            ? censorNumber(wlWinner.visitor?.mobile)
                            : wlWinner.visitor?.mobile}
                        </Text>
                        <Center>
                          <a
                            href="#"
                    ...
                    {winners.map((winner) => (
                      ...
                      >
                        ...
                        <Text align="center">
                          {censored
                            ? censorNumber(winner.visitor?.mobile)
                            : winner.visitor?.mobile}
                        </Text>
                      </div>
                    ))}
                  </Group>
